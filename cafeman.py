...
def merge(left, right):
    ...
    while i < len(left) and j < len(right):
        # Sort by priority first, then by prep_time descending
...
def assign_waiter(order):
    # Greedy approach: pick waiter with least occupied_time
    available_waiter = min(waiters, key=lambda w: w["occupied_time"])
...
def show_waiter_status():
    print("\n=== WAITER STATUS ===")
    for w in waiters:
        print(f"{w['name']}: {w['occupied_time']} min busy")
    print("=====================")
...
def show_all_orders():
    ...
    print("\n=== ORDER LIST ===")
    for o in orders:
        print(f"#{o['id']} - {o['item']} (Priority {o['priority']}) → Waiter: {o['assigned_waiter']}")
    print("==================")
