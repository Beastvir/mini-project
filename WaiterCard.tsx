...
export function WaiterCard({ waiter }: WaiterCardProps) {
  const getStatusColor = () => {
    if (waiter.occupiedTime < 5) return { bg: '#E8F4E9', text: '#4CAF50', label: 'Free' };
    if (waiter.occupiedTime < 10) return { bg: '#FFF9E6', text: '#FFA726', label: 'Busy' };
    ...
    >
      <div className="flex items-center justify-center flex-col mb-4">
        ...
        <div className="text-center">
          <h4 className="text-center" style={{ color: '#6B4F4F' }}>{waiter.name}</h4>
          <span
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t" style={{ borderColor: '#EADBC8' }}>
          <div className="text-center">
            <div className="text-center" style={{ fontSize: '0.75rem', color: '#8B6F6F' }}>Current</div>
            <div className="text-center" style={{ color: '#6B4F4F' }}>{waiter.currentOrders}</div>
          </div>
          <div className="text-center">
            <div className="text-center" style={{ fontSize: '0.75rem', color: '#8B6F6F' }}>Total</div>
            <div className="text-center" style={{ color: '#6B4F4F' }}>{waiter.totalOrders}</div>
          </div>
        </div>
      </div>
    </motion.div>
